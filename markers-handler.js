// markers-handler.js  (v2025-09-29e)
(function () {
  console.log("[markers-handler] loaded v2025-09-29e");
    ...
    setTimeout(() => {
      try {
        ...
        if (window.searchSuggest && typeof window.searchSuggest.setQuery === "function") {
          window.searchSuggest.setQuery(query, true); // 바로 렌더/오픈
        } else if (typeof window.filterSelTxt === "function") {
          // sel_txt 필터 방식 폴백
          window.filterSelTxt(query);
        }
        // 어떤 경우든 리스너가 반응하도록 이벤트 송출
        ...
        (function(i){
          ...
          kakao.maps.event.addListener(marker, "mouseup", function(){
            ...
            setTimeout(function(){
              ...
              // === 검색어: 규칙에 따라 추출 ===
              //    "쓰-011-가야한전주변02(회전)" -> "가야한전주변"
              let q = extractSearchName(pos.content) || extractSearchName(el.textContent || "");
              if (!q) {
                const t = document.createElement("div");
                t.innerHTML = String(pos.content ?? "");
                q = extractSearchName(t.textContent || "");
              }
              pushToSearchUI(q);
